...
{
    /// <summary>
    /// Handles the manipulation and retrieval of registrants from the database.
    /// </summary>
    public class RegistrantRepository
        : IRepository<Registrant, Guid>, IDisposable
    {
        ...
        protected HashSet<Guid> _updatedRegistrants;
        ...
        /// </summary>
        public RegistrantRepository()
        {
            this._context = new EFDbContext();
            this._context.AddModifiedDelegate(UpdateRegistrants);
            this._context.AddAddedItemDelegate(UpdateRegistrants);
            this._context.AddOnSaveCompleteDelegate(OnSaveComplete);
            this._contextInScope = true;
            this._updatedRegistrants = new HashSet<Guid>();
        }
        ...
        public RegistrantRepository(User user, IPrincipal principal)
            : this()
        {
        ...
        public RegistrantRepository(EFDbContext context)
        {
        ...
        /// <param name="user">The user that is accessing the database.</param>
        /// <param name="principal">The logged in principal for this repository.</param>
        public RegistrantRepository(EFDbContext context, User user, IPrincipal principal)
            : this(context)
        {
            if (user != null && principal != null)
                _context.SecuritySettings.SetUser(user, principal);
        ...
        /// <summary>
        /// Removes non permissives from a set of registrants.
        /// </summary>
        ...
        public IEnumerable<Registrant> RemoveNonPermissive(IEnumerable<Registrant> nodes, SecurityAccessType accessType)
        {
            var registrants = new List<Registrant>();
            foreach (var node in nodes)
            {
                if (this._context.CanAccess(node, accessType, nodes.Count() > 1))
                    registrants.Add(node);
            }
            return registrants;
        }
        ...
        public Registrant RemoveNonPermissive(Registrant node, SecurityAccessType accessType)
        {
            var reg = RemoveNonPermissive(new List<Registrant>() { node }, accessType).FirstOrDefault();
            if (reg == null)
        ...
        /// <param name="accessType">The type of access happening.</param>
        /// <returns>A set of registrants that can be used for the desired access type.</returns>
        public async Task<IEnumerable<Registrant>> RemoveNonPermissiveAsync(IEnumerable<Registrant> nodes, SecurityAccessType accessType)
        {
        ...
        /// <param name="accessType">The type of access happening.</param>
        /// <returns>The registrant or null.</returns>
        public async Task<Registrant> RemoveNonPermissiveAsync(Registrant node, SecurityAccessType accessType)
        {
            return await RemoveNonPermissiveAsync(node, accessType);
        }

        /// <summary>
        /// Removes all the registrants from <code>_updatedRegistrants</code>.
        /// </summary>
        public void OnSaveComplete()
        {
            this._updatedRegistrants.Clear();
        }

        /// <summary>
        /// Upates the registrants.
        /// </summary>
        /// <param name="nodes">The registrants to update.</param>
        public void UpdateRegistrants(IEnumerable<DbEntityEntry> nodes)
        {
            foreach (var entry in nodes.Where(e => e.Entity is Registrant))
            {
                _updatedRegistrants.Add(((Registrant)entry.Entity).UId);
                UpdateRegistrant(this._context.Entry((Registrant)entry.Entity));
            }
            foreach (var entry in nodes.Where(e => e.Entity is IRegistrantItem))
            {
                if (_updatedRegistrants.Contains(((IRegistrantItem)entry.Entity).Registrant.UId))
                    continue;
                UpdateRegistrant(this._context.Entry(((IRegistrantItem)entry.Entity).Registrant));
            }
        }
        ...
        public void UpdateRegistrant(DbEntityEntry<Registrant> entry)
        {
            try
            {
                ...
                if (this._context.SecuritySettings.AppUser != null)
                    modifier = this._context.SecuritySettings.AppUser.UId;
                ...
                var origStatus = (origValues["Status"] as RegistrationStatus?).Value;
                if (origStatus != registrant.Status)
                    registrant.StatusDate = DateTimeOffset.Now;
                if (origStatus == RegistrationStatus.Incomplete && registrant.Status == RegistrationStatus.Submitted)
        ...
        public Registrant Find(Guid key, bool usePermissions = true)
        {
            ...
            return this._PostProcessing(registrant);
        }
        ...
        public Registrant First(Expression<Func<Registrant, bool>> search, bool usePermissions = true)
        {
            ...
            if (registrant == null)
                throw new RegistrantNotFoundException();
            if (usePermissions)
                return this._PostProcessing(RemoveNonPermissive(registrant, SecurityAccessType.Read));
            return this._PostProcessing(registrant);
        }
        ...
        public IEnumerable<Registrant> Search(Expression<Func<Registrant, bool>> search, bool usePermissions = true)
        {
            ...
            return this._PostProcessing(registrants);
        }
