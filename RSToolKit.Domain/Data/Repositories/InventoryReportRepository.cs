...
namespace RSToolKit.Domain.Data
{
    public class InventoryReportRepository
        : IRepository<AdvancedInventoryReport, Guid>
    {
        ...
        public InventoryReportRepository(User user, IPrincipal principal)
            : this()
        {
        ...
        public InventoryReportRepository(EFDbContext context)
        {
        ...
        /// <param name="user">The user that is accessing the database.</param>
        /// <param name="principal">The logged in principal for this repository.</param>
        public InventoryReportRepository(EFDbContext context, User user, IPrincipal principal)
            : this(context)
        {
            if (user != null && principal != null)
                _context.SecuritySettings.SetUser(user, principal);
        ...
        public IEnumerable<AdvancedInventoryReport> RemoveNonPermissive(IEnumerable<AdvancedInventoryReport> nodes, SecurityAccessType accessType)
        {
            var forms = new List<AdvancedInventoryReport>();
        ...
        public AdvancedInventoryReport RemoveNonPermissive(AdvancedInventoryReport node, SecurityAccessType accessType)
        {
            var form = RemoveNonPermissive(new List<AdvancedInventoryReport>() { node }, accessType).FirstOrDefault();
            if (form == null)
        ...
        /// <param name="accessType">The type of access happening.</param>
        /// <returns>A set of transaction requests that can be used for the desired access type.</returns>
        public async Task<IEnumerable<AdvancedInventoryReport>> RemoveNonPermissiveAsync(IEnumerable<AdvancedInventoryReport> nodes, SecurityAccessType accessType)
        {
        ...
        public async Task<AdvancedInventoryReport> RemoveNonPermissiveAsync(AdvancedInventoryReport node, SecurityAccessType accessType)
        {
        ...
        /// <param name="usePermissions">User permissions or not.</param>
        /// <returns>The seater found.</returns>
        public AdvancedInventoryReport FindSeater(long key, bool usePermissions = true)
        {
            var node = Context.AdvancedInventoryReports.FirstOrDefault(s => s.SortingId == key);
            if (node == null)
                throw new FormNotFoundException();
            if (this._context.CanAccess(node, SecurityAccessType.Read))
                return node;
        ...
        public AdvancedInventoryReport Find(Guid key, bool usePermissions = true)
        {
            var node = Context.AdvancedInventoryReports.Find(key);
            if (node == null)
        ...
        /// <exception cref="TransactionNotFoundException">Throws if the transaction request was not found.</exception>
        /// <exception cref="InsufficientPermissionsException">Throws if not enough permissions to read transaction request.</exception>
        public AdvancedInventoryReport First(Expression<Func<AdvancedInventoryReport, bool>> search, bool usePermissions = true)
        {
            var node = Context.AdvancedInventoryReports.FirstOrDefault(search);
            if (node == null)
                throw new TransactionNotFoundException();
            if (usePermissions)
                return RemoveNonPermissive(node, SecurityAccessType.Read);
        ...
        /// <exception cref="TransactionNotFoundException">Throws if the transaction request was not found.</exception>
        /// <exception cref="InsufficientPermissionsException">Throws if not enough permissions to read transaction request.</exception>
        public async Task<AdvancedInventoryReport> FindAsync(Guid key, bool usePermissions = true)
        {
            var node = await Context.AdvancedInventoryReports.FindAsync(key);
            if (node == null)
                throw new TransactionNotFoundException();
            if (usePermissions)
                return RemoveNonPermissive(node, SecurityAccessType.Read);
        ...
        public IEnumerable<AdvancedInventoryReport> Search(Expression<Func<AdvancedInventoryReport, bool>> search, bool usePermissions = true)
        {
            var nodes = Context.AdvancedInventoryReports.Where(search).ToList().AsEnumerable();
            if (usePermissions)
        ...
        /// <param name="usePermissions">Set to false to skip permission checks.</param>
        /// <returns>An enumeration of transaction requests.</returns>
        public async Task<IEnumerable<AdvancedInventoryReport>> SearchAsync(Expression<Func<AdvancedInventoryReport, bool>> search, bool usePermissions = true)
        {
            return await Task.Run(() =>
            {
        ...
        public bool Remove(AdvancedInventoryReport node, bool usePermissions = true)
        {
            if (node == null)
                throw new TransactionNotFoundException();
            if (usePermissions)
        ...
        public async Task<bool> RemoveAsync(AdvancedInventoryReport node, bool usePermissions = true)
        {
            return await Task.Run(() =>
            {
                return Remove(node);
        ...
        /// <param name="usePermissions">Set to false to skip permission checks.</param>
        /// <returns>True if it did.</returns>
        public bool RemoveRange(IEnumerable<AdvancedInventoryReport> nodes, bool usePermissions = true)
        {
            if (usePermissions)
        ...
        /// <param name="usePermissions">Set to false to skip permission checks.</param>
        /// <returns>True if it did.</returns>
        public async Task<bool> RemoveRangeAsync(IEnumerable<AdvancedInventoryReport> nodes, bool usePermissions = true)
        {
            return await Task.Run(() =>
            {
        ...
        protected void _RemoveInventoryReport(AdvancedInventoryReport node)
        {
            if (node == null)
                return;
            ...
            this._context.RemoveAdvancedInventoryReport(node);
        }
        ...
        protected void _RemoveFormRange(IEnumerable<AdvancedInventoryReport> nodes)
        {
            foreach (var node in nodes)
                _RemoveInventoryReport(node);
        }
        ...
        public void Add(AdvancedInventoryReport node, bool userPermissions = true)
        {
            node = RemoveNonPermissive(node, SecurityAccessType.Write);
            if (node == null)
                return;
            this._context.AdvancedInventoryReports.Add(node);
        }
