        ...
        $.ajax({
            ...
            success: function (jsonResult) {
                if (jsonResult.Success) {
                    window.location = "EditReport?cid=" + CurrentCompany + "&rid=" + jsonResult.UId;
                }
                else {
        ...
        success: function (jsonResult) {
            if (jsonResult.Success) {
                ...
                for (var i = 0; i < reports.length; i++) {
                    var date = new moment(reports[i].DateCreated);
                    information += '<tr><td><a href="' + window.location.origin + '/Cloud/PagedReport/' + reports[i].UId + '">' + reports[i].Name + '</a></td><td>' + date.format('YYYY MM DD HH:mm A') + '</td></tr>';
                }
